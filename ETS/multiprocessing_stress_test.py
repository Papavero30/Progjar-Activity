...
class MultiprocessingStressTest:
    ...
    def run_comprehensive_test(self, server_workers):
        ...
        print(f"Starting Multiprocessing Stress Test (Server Workers: {server_workers})")
        print("=" * 80)
        ...
        for operation in operations:
            for file_size in file_sizes:
                for num_clients in client_workers:
                    ...
                    try:
                        result_multiprocessing = self.run_single_test(
                            operation, file_size, num_clients
                        )
                        result_multiprocessing.update({
                            'no': test_num,
                            'operation': operation,
                            'volume_mb': file_size,
                            'client_workers': num_clients,
                            'server_workers': server_workers,
                            'concurrency_type': 'multiprocessing'
                        })
                        results.append(result_multiprocessing)
                        
                        print(f"Multiprocessing - Success: {result_multiprocessing['client_success']}, Failed: {result_multiprocessing['client_failed']}")
                        
                    except Exception as e:
                        print(f"Multiprocessing test failed: {e}")
                        results.append(self.create_error_result(test_num, operation, file_size, num_clients, server_workers, 'multiprocessing', str(e)))
    ...
    def run_single_test(self, operation, file_size_mb, num_workers):
        ...
        results = self.run_multiprocessing_test(operation, filepath, filename_base, num_workers)
        end_time = time.time()
    ...
    def pre_upload_files(self, filepath, filename_base, num_workers):
        ...
        try:
            ...
            for batch in batches:
                ...
                # Process each batch sequentially when server has few workers
                for i in batch:
                    print(f"  Uploading file {i+1}/{num_workers}...", end="", flush=True)
                    command = f"UPLOAD {filename_base}_{i}.dat {file_content}"
    ...
    def run_multiprocessing_test(self, operation, filepath, filename_base, num_workers):
        """Run test using multiprocessing pool"""
        results = []
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for i in range(num_workers):
                if operation == 'upload':
                    future = executor.submit(upload_worker_func, i, filepath, filename_base, self.server_address)
                elif operation == 'download':
                    future = executor.submit(download_worker_func, i, filename_base, self.server_address)
            ...
            for future in as_completed(futures):
                try:
                    ...
                    results.append({'success': False, 'error': str(e), 'duration': 0, 'bytes': 0})
    ...
    def save_results_to_csv(self, results, server_workers):
        ...
        filename = f'multiprocessing_test_results_{server_workers}_workers_{int(time.time())}.csv'
    ...
    def print_summary_table(self, results):
        ...
        print(f"\n{'='*150}")
        print("MULTIPROCESSING STRESS TEST RESULTS")
        print(f"{'='*150}")
...
def send_command_mp(command_str, server_address, timeout=60):
    """Send command in multiprocessing context"""
    sock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sock.settimeout(timeout)
...
def upload_worker_func(worker_id, filepath, filename_base, server_address):
    """Upload worker for multiprocessing"""
    start_time = time.time()
    ...
    try:
        ...
        result = send_command_mp(command, server_address, timeout=300)  # Increased timeout
...
def download_worker_func(worker_id, filename_base, server_address):
    """Download worker for multiprocessing"""
    start_time = time.time()
    
    try:
        command = f"GET {filename_base}_{worker_id}.dat"
        result = send_command_mp(command, server_address, timeout=120)
...
def main():
    parser = argparse.ArgumentParser(description='Multiprocessing Stress Test for File Server')
    ...
    print("File Server Multiprocessing Stress Test")
    print("====================================")
    ...
    test = MultiprocessingStressTest(server_address)
    results = test.run_comprehensive_test(args.server_workers)
    
    print(f"\n✓ Multiprocessing stress test completed!")
